...
ui <- fluidPage(
  ...
  ## appFrameHeaderFixed(),
  titlePanel(title=div(img(src="logo.png", height=120, width=104), "Observer coverage simulator - 3.3.0"),
  ...
  tabsetPanel(id="alltabs",
                         ...
                         column(3, wellPanel(
                           ...
                           textInput(inputId="d.ppos",
                                     label=HTML(paste("Dispersion index for BPUE",
                                                      ...
                                                      " and turtles, but higher d may be needed, e.g., when ",
                                                      "animals are highly aggregated or variation among ",
                                                      "vessels or trips is high. Seabirds and fishes tend ",
                                                      "to be more skewed.)",
                                                      "</div>")),
                                     value="2",
              ...
              tabPanel("Objective: Bycatch Estimation CV", value="cv",
                       fluidRow(
                         column(3, wellPanel(
                           textInput(inputId="te.cv",
                                     label=HTML(paste("Total effort in fishery (e.g., trips)")),
                                     value="500",
                                     placeholder="Integer > 1"),
                           textInput(inputId="bpue.cv", label="Bycatch per Unit Effort (BPUE)",
                                     value="0.01",
                                     placeholder="Number > 0"),
                           textInput(inputId="d.cv",
                                     label=HTML(paste("Dispersion index for BPUE",
                                                      "<div class='extext'>",
                                                      "(Dispersion index d is variance:mean",
                                                      " ratio of BPUE, with more skewed data having ",
